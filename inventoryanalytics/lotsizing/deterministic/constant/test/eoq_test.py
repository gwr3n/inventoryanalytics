...
class TestEOQ(unittest.TestCase):
    ...
    def test_sensitivity_to_Q(self):
        ...
        self.assertAlmostEqual(self.eoq.sensitivity_to_Q(Q), (self.eoq.cost(Q)-d*v)/(self.eoq.cost(Qopt)-d*v), places=2)

    def test_reorder_point(self):
        L = 1/12
        self.assertAlmostEqual(self.eoq.reorder_point(L), 200, places=2)
...
class TestEOQ_incremental_discounts(unittest.TestCase):
    ...
    def test_eoq(self):
        ...
        Q = pb.compute_eoq()
        self.assertAlmostEqual(Q, 304.04, places=2)
