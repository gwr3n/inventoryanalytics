...
def simulate_newsvendor():
    instance = {"inventory_level": 0, "fixed_ordering_cost": 0, "holding_cost": 1, "penalty_cost": 5}
    ...
    des.schedule(EventWrapper(EndOfPeriod(des, w)), 1) # schedule EndOfPeriod with a delay of one period
...
np.random.seed(1234)
replications = 1000
print("Simulated cost: " +str(sum([simulate_newsvendor() for k in range(replications)])/replications))
